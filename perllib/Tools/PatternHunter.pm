...
sub new {
	...
	my($this) = {};
	bless $this, $class;
	$this->{parser} = Tools::PatternHunterParser->new;
	$this;
}
###############################################################################
package Tools::PatternHunterResult;
###############################################################################
@ISA = qw(Tools::HomologyResult);

###############################################################################
package Tools::PatternHunterHSP;
###############################################################################
...
###############################################################################
package Tools::PatternHunterParser;
###############################################################################
...
sub readfile {
	my($this) = @_;
	my($result) = Tools::PatternHunterResult->new;
	my($ln);
	my($status);
	while ($_ = $this->getline) {
		...
		if (/^>/) {
			...
			$hsp = Tools::PatternHunterHSP->new;
			$hsp->{from1} = $from1;
			$hsp->{to1} = $to1;
			$hsp->{from2} = $from2;
			$hsp->{to2} = $to2;
...
package main;
if ($0 eq __FILE__) {
	$parser = Tools::PatternHunterParser->new($ARGV[0]);
